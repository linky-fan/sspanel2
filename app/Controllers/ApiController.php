...
class ApiController extends BaseController
{
    ...
    public function wxpay_callback($request, $response, $args)
    {
        ...
        $result = (array)simplexml_load_string($postStr, 'SimpleXMLElement', LIBXML_NOCDATA);
        if($result['result_code'] == 'SUCCESS'){
            $wxpay = Wxpay::where('buy_num', '=', $result['out_trade_no'])->first();
            $wxpay->is_buy = 2;
            $wxpay->save();
            if ($wxpay->ref_by != "" && $wxpay->ref_by != 0 && $wxpay->ref_by != null) {
                $gift_user = User::where("id", "=", $wxpay->ref_by)->first();
                $gift_user->money = ($gift_user->money + ( $result['total_fee'])/100 * (Config::get('code_payback') / 100));
                $gift_user->save();
                $is_first_payback = Payback::where('userid',$wxpay->userid)->first();;
                if(!$is_first_payback){
                    $Payback = new Payback();
                    $Payback->total =$result['total_fee']/100;
                    $Payback->userid = $wxpay->userid;
                    $Payback->ref_by = $wxpay->ref_by;
                    $Payback->ref_get =( $result['total_fee'])/100 * (Config::get('code_payback') / 100);
