...
class ShopController extends AdminController
{
    public function index($request, $response, $args)
    {
        $table_config['total_column'] = array("op" => "操作", "id" => "ID", "name" => "商品名称",
                        ...
                        "auto_renew" => "自动续费", "auto_reset_bandwidth" => "续费时重置流量",
            "status" => "状态", "type" => "用户类型");
    ...
    {
        $list  = ShopPackage::All();
        return $this->view()->assign("list",$list)->display('admin/shop/create.tpl');
    }
    ...
    public function edit($request, $response, $args)
    {
        ...
        $list  = ShopPackage::All();
        return $this->view()->assign('shop', $shop)->assign("list",$list)->display('admin/shop/edit.tpl');
    }
    ...
    public function ajax_shop($request, $response, $args)
    {
        ...
        $datatables->query('Select id as op,id,name,price,content,auto_renew,auto_reset_bandwidth,type,status from shop');
        ...
        $datatables->edit('type', function ($data) {
            $shopPackage = ShopPackage::find($data['type']);
            return $shopPackage['name'];
        });
