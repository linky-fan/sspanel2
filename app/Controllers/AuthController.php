...
class AuthController extends BaseController
{
    public function login()
    {
        ...
        return $this->view()->assign('geetest_html', $GtSdk)->assign('ann',$Ann)->assign('login_token', $login_token)->assign('login_number', $login_number)->assign('telegram_bot', Config::get('telegram_bot'))->display('auth/login.tpl');
    }
    ...
    public function loginHandle($request, $response, $args)
    {
        ...
        $rs['ret'] = 1;
        $rs['msg'] = "欢迎回来,您还有".floor((strtotime($user->expire_in)-time())/3600/24)."天到期!";
    ...
    public function register($request, $response, $next)
    {
        ...
        return $this->view()->assign('ann',$Ann)->assign('enable_invite_code', Config::get('enable_invite_code'))->assign('geetest_html', $GtSdk)->assign('enable_email_verify', Config::get('enable_email_verify'))->assign('code', $code)->display('auth/register.tpl');
    }
...
//        }
        ...
        if ($user->save()) {
            if($code != ''){
                $isCode = InviteCode::where('code', $code)->first();
               if($isCode['user_id'] != 0){
                   $Timeback = new Timeback();
                   $Timeback->userid = $user->id;
                   $Timeback->invite_code = $code;
                   $Timeback->ref_get = Config::get('timeback');
                   $Timeback->ref_by = $isCode['user_id'];
                   $Timeback->datetime = time();
                   $Timeback->save();
                   $isUser = User::where('id', $isCode->user_id)->first();
                   $isUser->expire_in = date("Y-m-d H:i:s", strtotime($isUser->expire_in) + Config::get('timeback') * 86400);
                   $isUser->save();
               }
            }
            $res['ret'] = 1;
